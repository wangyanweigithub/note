...
* [一. 概念](#一-概念)
	* [1. 声明](#1-声明)
* [二. 输入输出和基础](#二-输入输出和基础)
	* [printf和scanf](#printf和scanf)
		* [1. 打印浮点数时:%f,默认显示小数点后6位，如果强制%f显示小数点后p位数字，可以把.p放置在%和f之间。：%.2f](#1-打印浮点数时f默认显示小数点后6位如果强制f显示小数点后p位数字可以把p放置在和f之间2f)
		* [2. %m.pX格式：m和p都是可选的。如果省略p，m和p之间的小数点也要去掉。](#2-mpx格式m和p都是可选的如果省略pm和p之间的小数点也要去掉)
		* [3. scanf](#3-scanf)
		* [4. 易混淆](#4-易混淆)
		* [5. 读写字符串](#5-读写字符串)
		* [6. 流](#6-流)
		* [7. 文件操作](#7-文件操作)
	* [2. 基本数据结构](#2-基本数据结构)
		* [1. 整数：int，unsigned int，long，long long，char](#1-整数intunsigned-intlonglong-longchar)
		* [2. float， double](#2-float-double)
	* [3. 基本语法](#3-基本语法)
		* [1. 表达式](#1-表达式)
		* [2. 语句](#2-语句)
		* [2. 重复语句](#2-重复语句)
		...
		* [5. 空间语句](#5-空间语句)
		* [3. 函数](#3-函数)
		* [1. 调用函数时，是将实参的值复制给形参的。所以函数如果想改变函数体外的变量需要使用引用或者指针。](#1-调用函数时是将实参的值复制给形参的所以函数如果想改变函数体外的变量需要使用引用或者指针)
		* [2. 函数定义](#2-函数定义)
		* [3. 函数声明](#3-函数声明)
		* [4. <font color=green>C语言中，实际参数是通过值传递的</font>](#4-font-colorgreenc语言中实际参数是通过值传递的font)
		* [5. 实际参数](#5-实际参数)
		* [4. 程序终止](#4-程序终止)
		* [1. main 函数的返回类型是int](#1-main-函数的返回类型是int)
		* [2. main函数返回的值是状态码，在某些操作系统中程序终止时可以检测到状态码。](#2-main函数返回的值是状态码在某些操作系统中程序终止时可以检测到状态码)
		* [3. exit函数，<stdlib.h>头文件。 传递给exit的参数和main函数返回值具有相同含义。](#3-exit函数stdlibh头文件-传递给exit的参数和main函数返回值具有相同含义)
		* [4. main函数中的语句 return表达式；等价于 exit(表达式)](#4-main函数中的语句-return表达式等价于-exit表达式)
		* [5. return和exit的差异：不管那个函数调用exit函数都会导致程序终止，return语句仅当有main函数调用时才会导致程序终止。](#5-return和exit的差异不管那个函数调用exit函数都会导致程序终止return语句仅当有main函数调用时才会导致程序终止)
		* [6. reurn后可以跟表达式，return会先计算表达式再返回。return后可以跟三元操作符。](#6-reurn后可以跟表达式return会先计算表达式再返回return后可以跟三元操作符)
		* [5. 问答](#5-问答)
		* [1. 编译器把不跟圆括号的函数名看做是指向函数的指针。](#1-编译器把不跟圆括号的函数名看做是指向函数的指针)
		* [2. 为什么要提供函数原型，只要把所有函数的定义放置在main函数之前，不就没有问题了吗？](#2-为什么要提供函数原型只要把所有函数的定义放置在main函数之前不就没有问题了吗)
		* [3. 为什么可以留着数组中第一维的参数不进行说明，但是其他维数必须说明呢？](#3-为什么可以留着数组中第一维的参数不进行说明但是其他维数必须说明呢)
		* [4. 如果函数f1调用函数f2，而函数f2又调用了f1，这样合法吗？](#4-如果函数f1调用函数f2而函数f2又调用了f1这样合法吗)
	* [3. 主要语言构造](#3-主要语言构造)
		* [1. 指令](#1-指令)
		* [1. 所有指令都以#字符开始，指令默认只占一行，每条指令的结尾没有分号或其他特殊标记。](#1-所有指令都以字符开始指令默认只占一行每条指令的结尾没有分号或其他特殊标记)
		* [2. 函数](#2-函数)
		* [3. 语句](#3-语句)
		* [1. <font color=red>C语言规定每条语句都要以分号结尾,但是有一个例外：复合语句就不以分号结尾。</font>](#1-font-colorredc语言规定每条语句都要以分号结尾但是有一个例外复合语句就不以分号结尾font)
		* [2. <font color=green>指令因为多是一行，所以不用分号结尾，而语句可能跨越多行，需要表示分隔</font>](#2-font-colorgreen指令因为多是一行所以不用分号结尾而语句可能跨越多行需要表示分隔font)
		* [4. 注释](#4-注释)
		* [1. //单行](#1-单行)
		* [2. ```/*  */```可以有多种方式。](#2---可以有多种方式)
		* [5. 变量和赋值](#5-变量和赋值)
		...
		* [7. 标识符](#7-标识符)
		* [6. C89关键字](#6-c89关键字)
		* [1. 有关数据“模子”的关键字：void、char、short、int、long、float、double、sizeof；](#1-有关数据模子的关键字voidcharshortintlongfloatdoublesizeof)
		* [2. 有关数据符号的关键字：signed、unsigned；](#2-有关数据符号的关键字signedunsigned)
		* [3. 构造类型：struct、union、enum、typedef；](#3-构造类型structunionenumtypedef)
		* [4. 有关存储与生命周期的类型：auto、static、extern、register、const、volatile；](#4-有关存储与生命周期的类型autostaticexternregisterconstvolatile)
		* [5. 有关语句的类型：if、else、for、while、do、switch、case、fault、goto、break、continue、return](#5-有关语句的类型ifelseforwhiledoswitchcasefaultgotobreakcontinuereturn)
		* [7. C99关键字](#7-c99关键字)
		* [1. _Bool](#1-_bool)
		* [2. _Complex](#2-_complex)
		* [3. _Imaginary](#3-_imaginary)
		* [4. inline](#4-inline)
		* [5. restrict](#5-restrict)
		* [8. C11关键字](#8-c11关键字)
		* [1. _Alignas](#1-_alignas)
		* [2. _Alignof](#2-_alignof)
		* [3. _Atomic](#3-_atomic)
		* [4. _Generic](#4-_generic)
		* [5. _Noreturn](#5-_noreturn)
		* [6. _Static_assert](#6-_static_assert)
		* [7. _Thread_local](#7-_thread_local)
		* [9. 程序结构](#9-程序结构)
		* [1. 局部变量](#1-局部变量)
		* [2. 外部变量](#2-外部变量)
		* [3. 程序块](#3-程序块)
...
* [三.高级数据结构](#三高级数据结构)
	...
	* [2. 结构、联合、枚举](#2-结构联合枚举)
		...
		* [3. 联合](#3-联合)
		* [3. 枚举](#3-枚举)
* [四. 字符串处理：替换，查找、截断](#四-字符串处理替换查找截断)
	* [1. 字符基础](#1-字符基础)
		* [1. 字符字面量](#1-字符字面量)
		* [2. 字符串变量](#2-字符串变量)
		* [3. 字符串的库函数](#3-字符串的库函数)
	* [2. Regex](#2-regex)
	* [3. Parser](#3-parser)
	* [<font color=green>4. 字符编码</font>](#font-colorgreen4-字符编码font)
* [五. 基本面向对象或函数式编程的特征](#五-基本面向对象或函数式编程的特征)
	* [1. 继承](#1-继承)
	* [2. 多态](#2-多态)
	* [3. Lambda 函数](#3-lambda-函数)
* [六. 调试：TDD技能](#六-调试tdd技能)
	* [1. 异常](#1-异常)
	* [2. 错误处理](#2-错误处理)
	* [3. 断言](#3-断言)
	* [4. 日志](#4-日志)
	* [5. 调试支持](#5-调试支持)
	* [<font color=green>6. 单元测试</font>](#font-colorgreen6-单元测试font)
* [<font color=green>七. 运行时</font>](#font-colorgreen七-运行时font)
	* [1. 程序代码和可执行代码的组织机制](#1-程序代码和可执行代码的组织机制)
	* [2. 运行时模块加载](#2-运行时模块加载)
	* [3. 符号查找机制](#3-符号查找机制)
* [八. 流](#八-流)
	* [1. 基本输入输出](#1-基本输入输出)
	* [2. 文件处理](#2-文件处理)
	* [3. 输入输出流类的组织](#3-输入输出流类的组织)
* [九. 事件驱动](#九-事件驱动)
	* [1. callback 方法调用](#1-callback-方法调用)
	* [2. 如何支持事件驱动编程模型](#2-如何支持事件驱动编程模型)
		* [1. .NET 的delegate，Java的anonymous inner class，Java 7的closure，C++OX的tr1::function/bind……等各种语言都会在这里下足功夫。](#1-net-的delegatejava的anonymous-inner-classjava-7的closurecox的tr1functionbind等各种语言都会在这里下足功夫)
* [十. 序列化和反序列化](#十-序列化和反序列化)
	* [1. 缺省机制](#1-缺省机制)
* [十一. regex和XML处理问题，可以跳过](#十一-regex和xml处理问题可以跳过)
* [十二. 并发](#十二-并发)
	* [1. 线程](#1-线程)
	* [2. 进程](#2-进程)
	* [3. 并发](#3-并发)
	* [4. 异步调用](#4-异步调用)
* [十三. <font color=green>动态编程</font>](#十三-font-colorgreen动态编程font)
	* [1. 反射](#1-反射)
	* [2. 元数据编程](#2-元数据编程)
	* [3. 数据和程序之间的相互转化机制](#3-数据和程序之间的相互转化机制)
	* [4. 运行时编译和执行的机制](#4-运行时编译和执行的机制)
* [十四. 语言常用的特色feature](#十四-语言常用的特色feature)
	* [1. 指针](#1-指针)
		* [1. 指针变量](#1-指针变量)
		* [2. 指针变量的声明](#2-指针变量的声明)
		* [3. 取地址符](#3-取地址符)
		* [4. 指针赋值](#4-指针赋值)
		* [5. 可以const来指明传递给函数的指针所指向的值不可改变。](#5-可以const来指明传递给函数的指针所指向的值不可改变)
		* [6. 指针可以作为函数返回值，但不能指向自动局部变量。可一指向全局变量或者static局部变量。](#6-指针可以作为函数返回值但不能指向自动局部变量可一指向全局变量或者static局部变量)
		* [7. <font color=green>C函数调用是值传递的。参数是指针时也是值传递，只不过指针的值是一个地址，赋值的指针值还是指针指向的对象的地址。所以改变p指向的对象时，会影响到外部数据，但是改变指针本身的地址，不会影响外部的指针。外部的指针还是指向原来对象</font>](#7-font-colorgreenc函数调用是值传递的参数是指针时也是值传递只不过指针的值是一个地址赋值的指针值还是指针指向的对象的地址所以改变p指向的对象时会影响到外部数据但是改变指针本身的地址不会影响外部的指针外部的指针还是指向原来对象font)
		* [8. const int *p 和 int * const p:](#8-const-int-p-和-int--const-p)
	* [2. 指针与数组](#2-指针与数组)
		* [1. C语言啊支持3中(而且只有3种)格式的指针算术运算：](#1-c语言啊支持3中而且只有3种格式的指针算术运算)
		* [2. 指针用于数组处理](#2-指针用于数组处理)
		* [3. 用数组名作为指针](#3-用数组名作为指针)
		* [4. 指针和多维数组](#4-指针和多维数组)
		* [5. C99 中的指针和变长数组](#5-c99-中的指针和变长数组)
		* [6. 问答](#6-问答)
	* [3. 指针的高级应用](#3-指针的高级应用)
		* [1. 动态存储分配](#1-动态存储分配)
		* [2. 动态分配字符串](#2-动态分配字符串)
		...
		* [7.问答](#7问答)
	* [4. 预处理器](#4-预处理器)
		* [1. 预处理器的最主要能力：宏定义、条件编译、文件包含。](#1-预处理器的最主要能力宏定义条件编译文件包含)
		* [2. 宏定义](#2-宏定义)
		* [3. 条件编译](#3-条件编译)
		* [4. 其他指令](#4-其他指令)
		* [5. 问答](#5-问答-1)
	* [5. 大型程序源文件结构](#5-大型程序源文件结构)
		* [1. 源文件](#1-源文件)
		* [2. 头文件](#2-头文件)
		* [3. 把程序划分为多个文件](#3-把程序划分为多个文件)
	* [6. 其他](#6-其他)
		* [1. c程序通常包含3个步骤](#1-c程序通常包含3个步骤)
		* [2. 在任何需要数值的地方，都可以使用具有相同类型的表达式。](#2-在任何需要数值的地方都可以使用具有相同类型的表达式)
		* [3. 宏](#3-宏)
		* [4. 代码规范](#4-代码规范)
* [十六. C/C++ 需要考虑内存模型](#十六-cc-需要考虑内存模型)
	* [1. 字面值的内存位置](#1-字面值的内存位置)
